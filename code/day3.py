...
class Bingo:
    ...
    def check_for_win(self) -> bool:
        for i in range(0, 5):
            if all(self.drawn[i * 5 + j] for j in range(5)) or all(
                self.drawn[j * 5 + i] for j in range(5)
            ):
                self.winner = True
                return True
        return False
    ...
    def calculate_score(self):
        print(self.last_number)
        return (
            sum(self.field[i] for i in range(25) if not self.drawn[i])
            * self.last_number
        )


def read_file(filename: str = "day4data.txt") -> list:
    with open(f"{__file__.rstrip('day3.py')}{filename}", "r") as f:
